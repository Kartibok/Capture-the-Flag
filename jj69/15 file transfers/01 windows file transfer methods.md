...
![[Pasted image 20221120111454.png]]
...
```shell-session
James Pearson@htb[/htb]$ sudo impacket-smbserver share -smb2support /tmp/smbshare
...
```shell-session
James Pearson@htb[/htb]$ sudo impacket-smbserver share -smb2support /tmp/smbshare -user test -password test
...
```shell-session
James Pearson@htb[/htb]$ sudo pip3 install pyftpdlib
```
...
```shell-session
James Pearson@htb[/htb]$ sudo python3 -m pyftpdlib --port 21
...
```shell-session
James Pearson@htb[/htb]$ echo  | base64 -d > hosts
```

  Decode Base64 String in Linux

```shell-session
James Pearson@htb[/htb]$ md5sum hosts 
...
```shell-session
James Pearson@htb[/htb]$ pip3 install uploadserver
...
```shell-session
James Pearson@htb[/htb]$ nc -lvnp 8000
...
```shell-session
James Pearson@htb[/htb]$ echo <base64> | base64 -d -w 0 > hosts
```
...
```shell-session
James Pearson@htb[/htb]$ sudo pip install wsgidav cheroot
...
 ```shell-session
James Pearson@htb[/htb]$ sudo wsgidav --host=0.0.0.0 --port=80 --root=/tmp --auth=anonymous 
...
```shell-session
James Pearson@htb[/htb]$ sudo python3 -m pyftpdlib --port 21 --write
